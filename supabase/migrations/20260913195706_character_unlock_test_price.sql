-- Temporary test price for premium character unlocks.
-- Restore to 500 before release.
...
begin
  ...
  v_balance := v_earned - v_spent;
  if v_balance < 10 then raise exception 'NOT_ENOUGH_STRAWBERRIES'; end if;

  insert into public.character_unlocks(user_id, animal, price_paid)
  values (new.user_id, new.animal, 10);
